#!/usr/bin/env bash
# t728u-next4 live CPR — separate chip boxes, omarchy-flat density, no tmux strip bleed.
set -euo pipefail

repo_root="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
test_root="$(mktemp -d /tmp/cockpit-bench-cpr-next4.XXXXXX)"
mirror="$test_root/proctor"
runtime_dir="$test_root/runtime"
socket_root="$test_root/tmux"
session="cockpit-bench-cpr-next4"
socket="$test_root/cockpit-bench-cpr-next4.sock"
fixture_root="$repo_root/tests/fixtures/bench"
db="$mirror/db/runs.sqlite"
proof_dir="$repo_root/proofs"
stamp="$(date -u +%Y%m%d-%H%M%S)"
proof_txt="$proof_dir/bench-bar-t728u-next4-live-${stamp}.txt"

fail() {
  printf 'Bench CPR next4 FAIL: %s\n' "$*" >&2
  exit 1
}
...
prior_next3='d7c2f6f77555e2c61a775238ea77095a59edcab4ceac3e75dd7101e9c9f341c0'
[[ "$proof_hash" != "$prior_next3" ]] ||
  fail "next4 CPR sha256 must differ from next3 local CPR ($prior_next3)"

printf '%s\n' 'Bench CPR next4 live capture: PASS'
printf 'cpr_path=%s\n' "$proof_txt"
printf 'cpr_sha256=%s\n' "$proof_hash"
